...
void readSdLog() {
    ...
    if (f != NULL) {
        uartUsb.write("--- SD Card Log Content ---\r\n", 29);
        char ch;
        ...
        uartUsb.write("--- End of SD Log ---\r\n", 23);
        fclose(f);
    } else {
        uartUsb.write("No Log File Found\r\n", 19);
    }
    fs.unmount();
    sd.deinit();
}
...
int main() {
    ...
    int printCounter = 0, alarmCooldownCounter = 0;
    ...
    while (true) {
        ...
        int triggerAlarm = 0;
        if (activeDisplayMode == 'A' && tempWarning) triggerAlarm = 1;
        if (activeDisplayMode == 'B' && gasDetected) triggerAlarm = 1;
        ...
        if (triggerAlarm && alarmCooldownCounter == 0) {
            ...
            while (alarmActive == 1) {
                ...
                if (lockPrintCounter >= 100) {
                    ...
                    char lockBuffer[160];
                    sprintf(lockBuffer, "ALARM ACTIVE | Mode: %c | Temp: %d.%d C | Gas: %d | Limit: %d.%d\r\n", 
                            activeDisplayMode, tW, tF, (int)lockGasScaled, thW, thF);
                ...
                if (lockKey != '\0') {
                    if (lockKey == '*') {
                        if (digitCount == 3 && inputSequence[0] == '1' && inputSequence[1] == '2' && inputSequence[2] == '3') {
                            alarmActive = 0; passiveSiren.write(0.0); activeDisplayMode = 'A'; digitCount = 0;
            ...
            if (printCounter >= 100) {
                ...
                char buffer[160];
                sprintf(buffer, "System Normal | Mode: %c | Temp = %d.%d C | Gas Level = %d | Limit = %d.%d\r\n", 
                        activeDisplayMode, tW, tF, (int)gasScaled, thW, thF);
