...
int main() {
    ...
    while (true) {
        ...
        if ((tempWarning == 1 || gasDetected == 1) && alarmCooldownCounter == 0) {
            alarmActive = 1;
            ...
            if (tempWarning == 1 && gasDetected == 1) {
                uartUsb.write("Buzzer Activated - Cause: [Temperature/Gas]\r\n", 45);
            } else if (tempWarning == 1) {
                ...
                if (lockKey != '\0') {
                    if (lockKey == '*') {
                        if (digitCount == 3 && inputSequence[0] == '1' && inputSequence[1] == '2' && inputSequence[2] == '3') {
                            ...
                            passiveSiren.write(0.0);
                            digitCount = 0;
                            uartUsb.write("Code Correct. Resetting Environmental Status...\r\n", 48);
            ...
            if (printCounter >= 100) {
                ...
                sprintf(buffer, "Temperature = %d.%d C  |  Gas Level = %d  |  Current Threshold = %d.%d  |  Status = Normal\r\n", 
                        tempWhole, tempFraction, gasWhole, threshWhole, threshFraction);
